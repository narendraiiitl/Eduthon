...
const mediaServer = mediasoup.Server({
    numWorkers: 1, // Use as many CPUs as available.
...
const checkAuth = async (client, data, cb) => {
    ...
    if (data.token === 'highSekurityToken')
        cb(null, {data})
    else {
...
io.on('connection', (socket) => {
    ...
    socket.on('mediasoup-notification', (notification) => {
        console.log('Got notification from client peer', notification);
